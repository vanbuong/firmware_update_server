...
- 🎯 Entry points to execute your program using `python -m <firmware_update_server>` or `$ firmware_update_server` with basic CLI argument parsing.
...
---
# firmware_update_server Flask Application

Awesome firmware_update_server created by vanbuong
...
```bash
git clone https://github.com/vanbuong/firmware_update_server firmware_update_server
cd firmware_update_server
make install
```
...
```bash
pip install firmware_update_server
```
...
```bash
$ python -m firmware_update_server
```
...
```bash
firmware_update_server create-db   # run once
firmware_update_server populate-db  # run once (optional)
firmware_update_server add-user -u admin -p 1234  # ads a user
firmware_update_server run
```
