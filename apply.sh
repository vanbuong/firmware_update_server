...
original_author="vanbuong"
original_name="firmware_update_server"
original_urlname="firmware_update_server"
original_description="Awesome firmware_update_server created by vanbuong" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/firmware_update_server "${name}"
...
then
    ...
    echo "then run 'firmware_update_server' or 'python -m firmware_update_server'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'firmware_update_server' or 'python -m firmware_update_server' or 'poetry run firmware_update_server'"
fi
